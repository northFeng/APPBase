...
- (void)getOSSClientWithBlock:(APPBackBlock)blockClient {
    ...
    //自签名模式！只创建一次就行！ 把if语句注销掉
    //if (_ossClient) {
        //client不存在
        [APPHttpTool getWithUrl:@"请求oss各种key" params:@{} success:^(id  _Nonnull response, NSInteger code) {
            NSDictionary *dicOss = (NSDictionary *)response;
...
- (void)initClientWithDicJson:(NSDictionary *)dicOss {
    ...
    NSNumber *time = [dicOss objectForKey:@"expiration"];
    token.expirationTimeInGMTFormat = time.stringValue;
    ...
    //if (!_ossClient) {
        ...
        _ossClient = [[OSSClient alloc] initWithEndpoint:endpoint credentialProvider:credential2];
    //}
    
    //_ossClient.credentialProvider = credential2;//更新这个属性
}
...
- (void)uploadData:(NSData *)upData dataId:(NSString *)dataId pathType:(NSString *)filePath contentType:(NSString *)contentType blockResult:(APPBackBlock)blockResult {
    ...
    [self getOSSClientWithBlock:^(BOOL result, id idObject) {
        if (result) {
            //上传
            NSString *upId = dataId;
            if (filePath.length) {
                upId = [NSString stringWithFormat:@"%@/%@",filePath,dataId];
            }
            [self uploadToOSSWithData:upData upId:dataId contentType:contentType blockResult:blockResult];
        }else{
            blockResult(NO,@"上传失败");
        }
    }];
...
///上传OSS阿里云
- (void)uploadToOSSWithData:(NSData *)upData upId:(NSString *)upId contentType:(NSString *)contentType blockResult:(APPBackBlock)blockResult {
    ...
    put.bucketName = _dicOss[@"bucket"];//@"<bucketName>";
    put.objectKey = upId;//"folder/subfolder/fileName"  fileName为文字名， / 前面 都是路径
    if (contentType.length) {
        put.contentType = contentType;//默认类型:application/octet-stream  音频类型 @"audio/mp3" 否则上传的链接为下载链接不能在线播放
    }
    ...
    [putTask continueWithBlock:^id(OSSTask *task) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!task.error) {
                ...
                blockResult(NO,[NSString stringWithFormat:@"上传云存储失败——>%@",task.error]);
             }
         });
         return nil;
     }];
